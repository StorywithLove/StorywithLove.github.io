...
- OCI 服务端 Agent API：通用时间序列查询、确定性分析加 DeepSeek 编排与解释，浏览器不保存模型密钥
...
页面右下角的分析助手会自动携带当前站点、所选日期、时区、指标和数据源。快捷
分析使用页面已加载功率；自由问题由 OCI Agent 解析成受约束的站点、时间、指标和
分组计划。`query_power` 自动分片查询，`analyze_power` 负责积分、峰值和完整率等
确定性计算，模型只解释紧凑结果。天气目前只有实时附近网格快照，因此不会伪造
